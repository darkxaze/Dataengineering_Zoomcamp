...
### Answer 1: 
'select * from myproject.my_nyc_tripdata.ext_green_taxi`
- dtc_zoomcamp_2025 and raw_nyc_tripdata are the default values but since we defined the env variable using myproject and my_nyc_tripdata those values will be used.
...
- env_var("DAYS_BACK", "30"): If no command-line argument is found, it falls
  back to the DAYS_BACK environment variable.
...
- The above image shows that for the file to materialize we need the taxi_zone_lookup to be compiled as well but for that we need to initialize the core files. dbt run --select models/staging/+ only complies staging and its downstream files.
...
### Answer 4:
 Everything except Setting a value for `DBT_BIGQUERY_STAGING_DATASET` env var is mandatory, or it'll fail to compile. The reason being if it is not present it defaults to the other case of DBT_BIGQUERY_TARGET_DATASET which has to be set.
